...
from smartify.agentic_development.mcp.core.cli_chat import CliChat
...
class CommandAutoSuggest(AutoSuggest):
    ...
    Args:
        prompts: Prompt definitions used to suggest command arguments.

    Attributes:
        prompts: Prompt definitions available to the CLI.
        prompt_dict: Lookup table keyed by prompt name.
    """
...
class UnifiedCompleter(Completer):
    ...
    Attributes:
        prompts: Prompt definitions available for command completion.
        prompt_dict: Lookup table keyed by prompt name.
        resources: Resource identifiers or metadata used for completion.
    """
...
class CliApp:
    ...
    Args:
        agent: Chat orchestrator responsible for prompt and response handling.

    Attributes:
        agent: Chat orchestrator used by the CLI loop.
        resources: Known document/resource identifiers for completion.
        prompts: Prompt definitions exposed by the backing MCP client.
        completer: Unified completer instance for command and resource entries.
        command_autosuggester: Autosuggestion provider for slash commands.
        kb: Prompt toolkit key bindings for interactive shortcuts.
        history: In-memory command history for the session.
        session: Prompt toolkit session used to gather user input.
    """
