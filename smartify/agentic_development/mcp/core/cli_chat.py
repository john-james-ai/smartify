...
from smartify.agentic_development.mcp.core.chat import Chat
...
class CliChat(Chat):
    """Extends chat handling with document and prompt-aware CLI helpers.

    Args:
        doc_client: MCP client used to read documents and resolve prompts.
        clients: Mapping of available MCP client names to client instances.
        claude_service: Claude service used to manage model interactions.
    ...
    async def list_prompts(self) -> list[Prompt]:
        """Returns available slash-command prompts from the document client.

        Returns:
            Prompt definitions exposed by the document MCP server.
        """
        return await self.doc_client.list_prompts()

    async def list_docs_ids(self) -> list[str]:
        """Returns available document identifiers.

        Returns:
            Document IDs that can be referenced from the CLI.
        """
        return await self.doc_client.read_resource("docs://documents")

    async def get_doc_content(self, doc_id: str) -> str:
        """Fetches the content for a specific document.

        Args:
            doc_id: Identifier of the requested document.

        Returns:
            Raw document content retrieved from the MCP resource.
        """
    ...
    async def get_prompt(
        ...
        """Fetches prompt messages for a slash command and document.

        Args:
            command: Slash command name without the leading slash.
            doc_id: Document identifier passed as prompt context.

        Returns:
            Prompt messages returned by the MCP prompt endpoint.
        """
...
def convert_prompt_message_to_message_param(
    ...
    """Converts an MCP prompt message into an Anthropic message param.

    Args:
        prompt_message: Prompt message returned by the MCP prompt API.

    Returns:
        Anthropic-compatible message content derived from the prompt message.
    """
...
def convert_prompt_messages_to_message_params(
    ...
    """Converts multiple MCP prompt messages into Anthropic message params.

    Args:
        prompt_messages: Prompt messages returned by the MCP prompt API.

    Returns:
        A list of Anthropic-compatible message params.
    """
    return [
